...
#TRAIN UNET FOR EACH SEED
for seed_nb in range(start_seed,end_seed+1):
    out_path = os.path.join(work_dir,"MODELS",model,area,sat,"STEP1","SEED_{}".format(seed_nb))
    ...
    print("seed",seed_nb)
    print("UNET setup")
    net = UNet(n_channels=nb_channels, n_classes=nb_classes, bilinear=True).to(device=device) 
    ...
    df_pre_train = pd.DataFrame(columns=['score_value','score_type','seed_nb','checkpoint'])
    ...
    for epoch in range(1,epochs+1):
        ...
        score_list = [pd.Series([val_loss, 'val_loss',seed_nb, checkpoint], index=df_pre_train.columns ) ,
                      pd.Series([train_loss, 'train_loss',seed_nb, checkpoint], index=df_pre_train.columns ) ]
        df_pre_train = df_pre_train.append(score_list, ignore_index=True)


        common_name = f'seed_{seed_nb}_step_1'
        df_plot = df_pre_train.query(f'seed_nb == {seed_nb}')
